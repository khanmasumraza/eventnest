...
const EventCard = ({
  ...
  // 🛡️ Guard: if no event object, render nothing
  if (!event) return null;

  const capacityLeft       = (event.capacity || 0) - (event.registered || 0);
  const capacityPercentage = event.capacity
    ? Math.min((event.registered / event.capacity) * 100, 100)
    : 0;
  const isFull    = capacityLeft <= 0;

  // 🛡️ FIX: safely get event ID — prevents /event/null
  ...
  const openEvent = () => {
    if (eventId) {
      ...
      console.error("EventCard: missing event ID for event:", event);
    }
  };
  ...
  const handleFavoriteClick = async (e) => {
    ...
    if (onToggleFavorite && eventId) onToggleFavorite(eventId);
  };
  ...
  // 🛡️ Capitalize city for display
  const displayCity = event.city
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 11, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ fontSize: 11, color: "#4b5563", margin: 0, fontWeight: 500 }}>
                {formattedDate}{displayCity ? ` · ${displayCity}` : ""}
              </p>
            </div>
          </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{
            ...
            <motion.div
              ...
              style={{ height: "100%", borderRadius: 99, background: progressColor }}
            />
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, position: "relative" }}>
          ...
          <motion.button
            ...
            onClick={e => {
              e.stopPropagation();
              if (!eventId) {
                console.error("EventCard: cannot register, event ID is missing");
                return;
              }
              onRegister(eventId);
            }}
            ...
            style={{
              ...
              opacity: !isAuthenticated && !isFull && !isRegistered ? 0.55 : 1,
              fontFamily: "'Plus Jakarta Sans', sans-serif",
